...
const registerUser = async (req, res) => {
  try {
    ...
    if (password.length < 8) {
      return res.json({ success: false, message: "Insira uma senha forte" });
    }
...
const bookAppointment = async (req, res) => {
  try {
    ...
    if (!barberData.available) {
      return res.json({ success: false, message: "Barbeiro indisponível" });
    }
    ...
    res.json({ success: true, message: "Agendamento Marcado" });
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: error.message });
  }
