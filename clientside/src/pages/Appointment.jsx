...
const Appointment = () => {
  ...
  const { barbers, currencySymbol, backendUrl, token, getBarbersData } =
  ...
  const fetchBarberInfo = async () => {
    const barberInfo = barbers.find((barber) => barber._id === barberId);
    ...
    for (let i = 0; i < 7; i++) {
      ...
      while (currentDate < endTime) {
        ...
        const isSlotAvailable =
          barberInfo.slots_booked[slotDate] &&
          barberInfo.slots_booked[slotDate].includes(slotTime)
            ? false
            : true;
  ...
  const bookAppointment = async () => {
    if (!token) {
      toast.warn("Faça login para agendar um horário");
    ...
    barberInfo && (
      <div>
        <div className="flex flex-col sm:flex-row gap-4">
          <div>
            <img
              className="bg-[#EFBF04] w-full sm:max-w-72 rounded-lg"
          ...
          <div className="flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0">
            ...
            <div className="flex items-center gap-2 text-sm mt-1 text-gray-600">
              <p>
                {barberInfo.degree} - {barberInfo.speciality}
              </p>
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
          ...
          <div className="flex items-center gap-3 w-full overflow-x-scroll mt-4">
            {barberSlots.length &&
              barberSlots[slotIndex].map((item, index) => (
                <p
          ...
          <button
            ...
            className="bg-[#EFBF04] text-white text-sm font-light px-14 py-3 rounded-full my-6"
          >
